...
void de_tick(de_t *de, uint64_t frame_count) {
    ...
    if (de->menu_open) apply_menu_to_widgets(de);
}
...
void de_render(de_t *de, struct srapi_framebuffer *fb) {
    ...
    /* Render the taskbar into its offscreen surface, then blit. We always do
       it because the clock or hover may have changed and we don't track every
       source of dirtiness exhaustively. */
    if (de->taskbar_ctx != NULL) {
        ranal_invalidate();
