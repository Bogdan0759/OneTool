...
static ranal_state_t g_default_ctx_storage;
ranal_state_t *g_ranal = &g_default_ctx_storage;
...
void ranal_event_pass_(void) {
    ...
    ranal_widget_t *hot = NULL;
    for (ranal_widget_t *p = g_ranal->popups; p != NULL && hot == NULL; p = p->next_sibling) {
        hot = ranal_hit_test_(p, g_ranal->mouse_x, g_ranal->mouse_y);
    }
    if (hot == NULL) {
        hot = ranal_hit_test_(g_ranal->root, g_ranal->mouse_x, g_ranal->mouse_y);
    }
    ...
    if (just_released) {
        ...
        for (ranal_widget_t *p = g_ranal->popups; p != NULL; p = p->next_sibling) {
            update_slider_drags(p, g_ranal->mouse_x, 0);
        }
    }
...
int ranal_render(void) {
    ...
    if (g_ranal->targets_drm) {
        fb = srapi_drm_backbuffer(g_ranal->drm);
    } else if (g_ranal->target != NULL) {
        fb = g_ranal->target->fb;
    }
    if (fb == NULL) {
        ranal_set_error_("ranal: render: no target");
        return 1;
    }
...
int ranal_present(void) {
    if (!g_ranal->initialized) return 1;
    if (g_ranal->targets_drm) {
        srapi_result_t r = srapi_drm_present(g_ranal->drm);
        if (r != SRAPI_OK) {
